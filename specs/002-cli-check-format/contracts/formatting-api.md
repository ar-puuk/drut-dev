...
- **`format`**: Parses `source` internally (the same way `parse` would), then
  ...
  baseline defaults to left untouched (`preserve`), or always normalized to
  column 0 when `options.top_level_indent` is explicitly `Normalize` —
  `008-top-level-indentation-normalization`, 2026-08-11, first reversed the
  original "left untouched" rule to always-normalize; `009-top-level-indent-
  toggle`, 2026-08-12, reverted that to `preserve` as the default while
  keeping `normalize` fully available, opt-in — continuation-line indentation
