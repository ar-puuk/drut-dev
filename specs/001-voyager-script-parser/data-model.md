...
| `closer` | `Option<Span>` | **Added post-hoc, during `002-cli-check-format` implementation** (2026-08-09) — the explicit closer statement's own span (`ENDIF`/`ENDLOOP`/`ENDRUN`/`ENDPROCESS`/`ENDPHASE`/`ENDJLOOP`/`ENDLINKLOOP`/`EndDistributeMULTISTEP`), or `None` when the block closed implicitly (`Run`/`Process`) or is genuinely unmatched. `span.end` alone can't distinguish "real closer" from "fell back to the last child's span" — this field exists because `002`'s formatter needs to know which case applies (to align an explicit closer with its opener, FR-012) without re-deriving block-matching logic outside this crate (constitution Principle I). Purely additive: no existing field changed meaning, no external code constructs a `Block` literal (grep-verified), so this required no `voyager-core` version bump beyond the ordinary addition of a struct field.
| `opener_pairs` | `Vec<Span>` | **Added alongside `closer`, same date/reason.** The opening statement's own `keyword=value` pair-keyword-name token spans (e.g. `PGM` and `ZONES` in `RUN PGM=MATRIX ZONES=5`) — not the pair's value content, and not the control word itself. Always empty for `If` in practice, since a condition's `=` comparisons sit inside parentheses (nonzero bracket depth) and are never mistaken for a pair. Exists for the same reason `closer` does: the opening `Statement` — including its `pairs` — is discarded once matched into a `Block`, so `002`'s casing rewrite (FR-015) has no other way to reach a block-opening statement's own pairs (e.g. normalizing `PGM`'s case in `RUN PGM=...`) without this. Also purely additive, same non-breaking verification as `closer`.
