...
mod tests {
    ...
    fn client_format_defaults_starts_at_default_before_any_pull() {
        let state = ServerState::new();
        assert_eq!(state.client_format_defaults().casing_control_words, None);
    ...
    fn client_format_defaults_round_trips_through_set() {
        ...
        let defaults = drut_config::ExplicitFormatOverride {
            casing_control_words: Some(voyager_core::CasingConvention::Upper),
            indent_width: Some(2),
            ..Default::default()
        };
        state.set_client_format_defaults(defaults);
        assert_eq!(state.client_format_defaults().casing_control_words, Some(voyager_core::CasingConvention::Upper));
    ...
    fn set_client_format_defaults_replaces_the_previous_cache_wholesale() {
        ...
        assert_eq!(state.client_format_defaults().casing_control_words, None, "the earlier pull's casing_control_words value must not linger");
