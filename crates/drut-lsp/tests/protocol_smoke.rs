...
fn did_open_publishes_a_drut_config_hint_for_a_malformed_drut_toml_additive_to_structural_diagnostics() {
    ...
    assert!(message.contains("casing_control_words"), "expected the message to name the specific bad key, got: {message}");
...
fn client_settings_apply_one_field_per_category_with_no_drut_toml_present() {
    ...
    // explicit flags (`--casing-control-words upper --indent-width 2
...
fn drut_toml_wins_for_its_own_field_while_a_client_setting_still_wins_for_a_field_drut_toml_never_sets() {
    ...
    // sets both indent_width (conflicting) and casing_control_words (which
    ...
    assert_eq!(
        ...
        "drut.toml's indent_width (4) must win over the client's (2); the client's \
         casing_control_words must still apply since drut.toml never sets casing at all"
    );
