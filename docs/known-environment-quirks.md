...
| 2026-08-11 | `target/debug/drut.exe` (`drut-cli`'s own debug binary), blocking `crates/drut-cli/tests/exit_codes.rs`'s entire 11-test suite (every test that spawns the compiled binary as a subprocess) | `D:\GitHub\drut-dev` (the trusted path), after merging `main` (with `006-unmatched-process-diagnostic`) into `005-format-on-save-paste` and rebuilding | Unlike the two rows above, this one did **not** self-clear on retry (re-ran `cargo test -p drut-cli --test exit_codes` twice more, ~15s apart, same block both times) and did **not** clear via direct shell invocation of the binary either (`Permission denied`, exit 126). What *did* immediately work, with zero relink/delete workaround needed: a **release** build of the identical code (`cargo build --release -p drut-cli`) ran cleanly on the first try and correctly exercised the new `UnmatchedProcess` diagnostic. Whether this reflects a real debug-vs-release asymmetry in the AC policy's heuristic (unsigned/unoptimized debug artifacts scored differently, e.g. by a reputation or signing check) or is coincidental is unconfirmed — but see the standing workaround below, adopted because it has now cost real time twice while release builds have never failed this session. |

**Standing workaround, adopted 2026-08-11**: for anything that spawns the actual
compiled `drut` binary as a subprocess — `exit_codes.rs`-style CLI tests, manual
verification, packaging for a `.vsix` build, or any other real invocation — default
to a **release** build (`cargo build --release`, `cargo test --release`) rather than
debug. Reserve debug builds for pure unit/lib tests that never shell out to the
compiled binary (`cargo test --lib`, `cargo test -p <crate>` without a subprocess-
spawning integration test) — those have never hit this quirk, in any of the rows
above. This isn't a proven root-cause fix (release binaries *have* been blocked
before, see the 2026-08-10 row) — it's a pragmatic default that has been reliable in
practice, adopted so a debug-binary-spawning test failing with no apparent logic
reason gets recognized on sight instead of re-diagnosed from scratch.
